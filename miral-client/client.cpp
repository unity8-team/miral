...
void updateAnimation (GTimer* timer, AnimationValues* anim)
{
    ...
    if (elapsed > 0.6f && elapsed < 6.833f)
        { if (anim->fadeBackground > 0.5) anim->fadeBackground -= 0.15f * dt; else anim->fadeBackground = 0.5; }
}
...
void window_event_callback(MirSurface* /*surface*/, MirEvent const* event, void* context)
{
    if (mir_event_get_type(event) == mir_event_type_close_surface)
    {
        static_cast<decltype(dying)*>(context)->store(true);
    }
}
...
std::shared_ptr<MirEglSurface> miral_eglapp_init(MirConnection* const connection)
{
    ...
    MirSurfaceParameters surfaceparm =
        {
            "miral-client",
            640, 480,
            select_pixel_format(connection),
    ...
    auto const mir_egl_app = make_mir_eglapp(connection, surfaceparm.pixel_format);

    return std::make_shared<MirEglSurface>(mir_egl_app, surfaceparm, swapinterval);
}

void run(MirConnection* const connection)
try
{
    GLuint prog[2];
    GLuint texture[2];
    ...
    auto const surface = miral_eglapp_init(connection);

    miral::toolkit::SurfaceSpec::for_changes(connection)
        .set_min_size(50, 50)
        .set_max_size(800, 600)
        .set_size_inc(10, 10)
        .set_event_handler(&window_event_callback, &dying)
        .apply_to(surface->mir_surface());
...
int main()
{
    miral::toolkit::Connection connection{mir_connect_sync(nullptr, "miral-client")};
    run(connection);
}